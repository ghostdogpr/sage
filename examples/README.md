...
zio/      …Example.scala + Tour   the ZIO tour
...
transaction, and classic pub/sub. The `…Example` objects are the individual copy-pasteable snippets the tour stitches together.
...
These need a TLS-enabled server or a master-replica deployment respectively, so they are not part of the localhost tours; they compile in CI
to keep the wiring honest and exist to be read.
