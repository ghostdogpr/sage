...
object CommandSamples {
  ...
  val all: Vector[Sample] = Vector(
    ...
    Sample(Server.configResetStat, Vector("CONFIG", "RESETSTAT")),
    Sample(Server.configRewrite, Vector("CONFIG", "REWRITE")),
    ...
    Sample(Server.lastSave, Vector("LASTSAVE")),
    ...
    Sample(Server.save, Vector("SAVE")),
    Sample(Server.bgSave(), Vector("BGSAVE")),
    Sample(Server.bgSave(schedule = true), Vector("BGSAVE", "SCHEDULE")),
    Sample(Server.bgRewriteAof, Vector("BGREWRITEAOF")),
    ...
    Sample(Acl.aclGenPass(), Vector("ACL", "GENPASS")),
    Sample(Acl.aclGenPass(Some(128)), Vector("ACL", "GENPASS", "128")),
    Sample(Acl.aclGetUser("default"), Vector("ACL", "GETUSER", "default")),
    Sample(Acl.aclSetUser("u", "on", ">pass", "~*", "+@all"), Vector("ACL", "SETUSER", "u", "on", ">pass", "~*", "+@all")),
    Sample(Acl.aclDelUser("u1", "u2"), Vector("ACL", "DELUSER", "u1", "u2")),
    Sample(Acl.aclDryRun("u", "GET", "k"), Vector("ACL", "DRYRUN", "u", "GET", "k")),
    Sample(Acl.aclLog(), Vector("ACL", "LOG")),
    Sample(Acl.aclLog(Some(10L)), Vector("ACL", "LOG", "10")),
    Sample(Acl.aclLogReset, Vector("ACL", "LOG", "RESET")),
    Sample(Acl.aclLoad, Vector("ACL", "LOAD")),
    Sample(Acl.aclSave, Vector("ACL", "SAVE")),
    ...
    Sample(Connection.clientPause(1.second), Vector("CLIENT", "PAUSE", "1000")),
    Sample(Connection.clientPause(1.second, Some(ClientPauseMode.Write)), Vector("CLIENT", "PAUSE", "1000", "WRITE")),
    Sample(Connection.clientUnpause, Vector("CLIENT", "UNPAUSE")),
    Sample(Connection.clientUnblock(42L), Vector("CLIENT", "UNBLOCK", "42", "TIMEOUT")),
    Sample(Connection.clientUnblock(42L, error = true), Vector("CLIENT", "UNBLOCK", "42", "ERROR"))
  )
}
