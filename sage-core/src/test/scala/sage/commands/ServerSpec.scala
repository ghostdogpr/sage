...
class ServerSpec extends munit.FunSuite {
  ...
  test("node-local metadata is explicit and survives decoder transformations") {
    assert(Server.configGet("*").nodeLocal)
    assert(Server.configSet("maxmemory" -> "100mb").nodeLocal)
    assert(Server.info().nodeLocal)
    assert(Server.role.nodeLocal)
    assert(Server.time.nodeLocal)
    assert(Server.memoryPurge.nodeLocal)
    assert(Server.slowLogLen.nodeLocal)
    assert(Server.commandCount.nodeLocal)
    assert(Server.commandList().nodeLocal)
    assert(Server.commandGetKeys("SET", "k", "v").nodeLocal)
    assert(Server.commandGetKeysAndFlags("GET", "k").nodeLocal)
    assert(Server.commandInfo("GET").nodeLocal)
    assert(Pubsub.pubsubChannels().nodeLocal)
    assert(Pubsub.pubsubNumPat.nodeLocal)
    assert(Acl.aclWhoAmI.nodeLocal)
    assert(Acl.aclList.nodeLocal)
    assert(Acl.aclUsers.nodeLocal)
    assert(Acl.aclCat().nodeLocal)
    assert(Acl.aclGetUser("default").nodeLocal)
    assert(Acl.aclLog().nodeLocal)
    assert(Connection.clientId.nodeLocal)
    assert(Connection.clientGetName.nodeLocal)
    assert(Connection.clientInfo.nodeLocal)
    assert(Connection.clientList.nodeLocal)
    assert(Connection.clientGetRedir.nodeLocal)
    assert(Functions.functionKill.nodeLocal)
    assert(Functions.functionDump.nodeLocal)
    assert(Functions.functionList().nodeLocal)
    assert(Functions.functionStats.nodeLocal)
    assert(Scripting.scriptKill.nodeLocal)
    assert(Scripting.scriptShow("sha").nodeLocal)
    assert(Keys.scan[String](ScanCursor.start).nodeLocal)
    assert(Server.info().map(identity).nodeLocal)
    assert(Server.info().rawFrame.nodeLocal)
    assert(!Server.dbSize.nodeLocal)
  }
