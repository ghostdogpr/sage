...
- Blocking commands are rejected everywhere in a transaction — `tx.run` and the inherited `tx.<blocking>` methods in the read phase, and `exec` through the Pipeline's existing guard — failing fast with `IllegalArgumentException`. The leased connection has no pool to absorb a block (unlike the client's `run`, which routes blocking commands to the Dedicated Pool), so a blocking command here would stall the scope and its fiber indefinitely; run such commands individually on the client instead.
